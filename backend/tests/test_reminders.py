...
def _process(minutes: int) -> int:
    async def _go() -> int:
        eng = create_async_engine(settings.database_url)
        session = async_sessionmaker(eng, expire_on_commit=False)
        try:
            async with session() as db:
                return await process_due_reminders(db, minutes=minutes)
        finally:
            await eng.dispose()
...
def test_reminder_sent_once_and_idempotent(
    ...
    assert _process(minutes=4320) == 1
    assert db_val(
        "SELECT count(*) FROM notification_logs WHERE notification_type='REMINDER'"
    ) == 1

    assert _process(minutes=4320) == 0  # already reminded
    assert db_val(
        "SELECT count(*) FROM notification_logs WHERE notification_type='REMINDER'"
    ) == 1
...
def test_reminder_window_excludes_far_future(
    ...
    assert _process(minutes=60) == 0  # slot is ~2 days out, not within 1h
...
def test_reminder_ignores_cancelled_event(
    ...
    assert _process(minutes=4320) == 0
